...
epochs_max = 229300 #Maximum training epochs
menu = [0] #optional multi value testing

#Input a label for the agent beign tested
label = input("Label for this agent")

#For each training option
for a in menu:
    if a == 0: #Selection for menu options
        agent = Murin(3**10,9,0.6,depth) #Make an agent
        print("Murin "+str(agent.Q.l)) #Print agent variables
        print("------------")
    ...
    while(epochs < epochs_max):
        ...
        deviance = 1.0-deviance #deviance measure
        ...
        for ap in range(9):
            ...
            if (p == k): #If on the first player turn
                ...
                act_p = a #Grab action
                aO,pO = optimal3(B) #get optimal agent move
                B[moves[int(aO)][0],moves[int(aO)][1]] = p #add to board
                C = check_lines(B) #Check lines
                B[moves[int(aO)][0],moves[int(aO)][1]] = 0 #set back from board

                win_pos = 0 #set win position to zero
                if (3*k in C):
                    win_pos = 1 #Set flag if found a win condition from optimal

                B[moves[int(a)][0],moves[int(a)][1]] = p #set to actually picked action
                C = check_lines(B) #Check lines for the actual move

                #If no winner actually and a win found from optimal, or a 2-string
                if (not(3*k in C) and win_pos) or (-2*k in C):
                    agent.m = 1 #Set agent depth shallow
                    Qo = agent.Q.l #Grab learn rate
                    agent.Q.l = 0.99 #Aggressive training
                    agent.train(0) #Train against fatal mistakes
                    agent.Q.l = Qo #restore learning rate
                    agent.m = depth #reset depth
                i = sum([b*(acts[b]==a) for b in range(len(acts))]) #extract out action choices
                del acts[i] #remove the action
...
#Dump data to file for analysis
pickle.dump(agent,open("3T_opt3_newtrain_"+label+".p","w"))
