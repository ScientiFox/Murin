...
#  A PC based source which allows the user to play Tic Tac Toe against a trained Murin
#  algorithm opponent. Thise version interfaces with an arduino system.
#
###
...
#Open the serial port - COM7 is an example
ser = serial.Serial("COM7")

#Load the agent brain
agent = pickle.load(open("T3_agent.p","rb"))
...
#Set up board and game variables
B = np.zeros((3,3))
...
#Note the system is online and send first message to device
print "Online"
ser.write("&")
...
while (1):
    ...
    #Wait for available data from the MCU
    while (ser.inWaiting()<15):
        pass

    #Receive from device, split, and print
    Bl = []
    s = ser.readline()
    s = s.split(",")
    print s
    ...
    for i in range(len(Bl)):
        if Bl[i] != 0:
            acts.remove(i) #Remove moves already used

    #Format board
    B = np.array([Bl[0:3],Bl[3:6],Bl[6:9]])

    #Check the lines and make the state
    C = check_lines(B)
    si = get_state(B)

    #Set agent depth
    agent.m = depth
    a = agent.act(si) #get agent action choice
    ...
    #aO,p = optimal3(B) #Alternative choice, from optimal solver
    ser.write(str(aO)) #Send action choice to device
